function draw_tab1_head2(){
  var categorical_color = ['tableau10','set3','accent','category10','dark2','paired','pastel1'];
  $('#c_tab1_d2').append(
    '<div id ="c_tab1_head2" style="padding-left:10px">'+
         '<h6 style="padding:5px; margin-bottom:0px">Node Size:</h6>'+
         // '<i class="ui " data-tooltip="Available for sequential property" data-position="bottom right">'+
         //
         //  '</i>'+
         '<div class="gap-line"></div>'+
          '<div>'+
               '<label class = "h1-l" style="margin:3px;margin-left:7px">Node Property:</label>'+
               '<select class="custom-select h2-sc con_property" id = "ch_ns_sc">'+
               '<option value="default" selected>Select</option>'+
               '</select>'+
          '</div>'+
         // ===========done with Size=========
         '<h6 style="padding:5px;margin-bottom:0px">Node Color:</h6>'+
         '<div class="gap-line"></div>'+

          '<div>'+
               '<label class = "h1-l" style="margin:3px;margin-left:7px">Node Property:</label>'+
               '<select class="custom-select h2-sc" id = "ch_nc_sc">'+
                  '<option class = "opt-g-font" value="default" selected>Select</option>'+
                  '<optgroup class ="con_property opt-g-font" label="Numerical Property">'+
                  '</optgroup>'+
                  '<optgroup class ="cat_property opt-g-font" label="Categorical Property">'+
                  '</optgroup>'+
               '</select>'+
          '</div>'+
          '<div>'+
...
// ========color map ============
             '<div  style="margin-left:54px;width:139px" class="h2-sc btn-group">'+
               '<button type="button" class="btn btn-secondary dpdown dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">'+
                '<img style = "height:12px;vertical-align:top;" class ="colormap" id = "nc_colormap" />'+
               '</button>'+
                 '<div class="dropdown-menu nc_menu">'+
        ...
        '</div>'+
                // ===========opacity=========
         '<h6 style="padding:5px;margin-bottom:0px">Node Opacity:</h6>'+
         '<div class="gap-line"></div>'+
          '<div>'+
             '<label class = "h1-l" style="margin:3px;margin-left:7px">Node Property:</label>'+
             '<select class="custom-select h2-sc con_property" id = "ch_nopa_sc">'+
             '<option value="default" selected>Select</option>'+
             '</select>'+
          '</div>'+
   // =============================node border-width
         ...
         '<div class="gap-line"></div>'+
          '<div>'+
             '<label class = "h1-l" style="margin:3px;margin-left:7px">Node Property:</label>'+
             '<select class="custom-select h2-sc con_property" id = "ch_nbw_sc">'+
             '<option value="default" selected>Select</option>'+
             '</select>'+
          '</div>'+
          // ============node- borde-color
            ...
            // ======= color map
              $(".nc_menu li").click(function(){
                ...
                if( categorical_color.indexOf(color)>=0){
                  ...
                  // console.log([color,color_m,eval(color)])
                  node_curr.attr('fill',function(d){
                    return eval(color)(d[prop])
                  })
                }
...
// =====================node size======
          $('#ch_ns_sc').change(function(){
              let prop = $(this).val();
              let dom = track_status[currentID]['mix']['v_nu'][prop];
              let scale = liScale.domain(dom).range([radi/2,radi*4])
              node_curr.attr("r",function(d){
                return scale(d[prop])});
          })

  // =====================node opacity======
            $('#ch_nopa_sc').change(function(){
                let prop = $(this).val();
                let dom = track_status[currentID]['mix']['v_nu'][prop];
                let scale = liScale.domain(dom).range([0.1,1])
                node_curr.style("opacity",function(d){
                  return scale(d[prop])});
            })
  // =====================node stroke-width======
